...
export class BasketService {
  ...
  private calculateTotals(): void{
    const basket = this.getCurrentBasketValue();
    const shipping = 0;
    // 0 is the initial value of result(accumulator of reduce callback)
    const subtotal = basket.items.reduce((result, crrItem) => (crrItem.price * crrItem.quantity) + result, 0);
